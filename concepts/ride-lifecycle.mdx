...
| `en_curso` | `conductor` | Driver has started the trip; passengers are being transported |
| `finalizado` | `conductor` | Trip has been completed successfully |
...
The `idle` state is a frontend-only concept used by the `useRideStore` Zustand store to represent the pre-request UI. It has no corresponding database row.
